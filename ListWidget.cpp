...
{	
	...
	{	
		for (int i = 0;	i < (int) mLines.size(); i++) 		
			if (stricmp(theLine.c_str(), mLines[i].c_str()) < 0) 
			{
				anIdx = i;
...
void ListWidget::SetColor(const std::string& theLine, const Color& theColor) 
{
	...
	if (anIdx != -1) 
	{
		...
		while (aListWidget != NULL) 
		{
			aListWidget->mLineColors[anIdx] = theColor;
			aListWidget->MarkDirty();
...
void ListWidget::MouseDown(int x, int y, int theClickCount) 
{
