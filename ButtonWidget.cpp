...
void ButtonWidget::Draw(Graphics* g)
{
	...
	if ((mFont == NULL) && (mLabel.length() > 0))
		mFont = new SysFont(mWidgetManager->mApp, "SansSerif", 10);
	...
	if ((mButtonImage == NULL) && (mDownImage == NULL))
	{
		...
		if (isDown)
		{
			...
			//TODO: this hard-coded color should be fixed
			g->SetColor(Color(0,0,0));
	...
	else
	{
		if (!isDown)
		{
			...
			{
				if (HaveButtonImage(mButtonImage, mNormalRect))
...
void ButtonWidget::MouseDown(int theX, int theY, int theClickCount)
{
...
void ButtonWidget::MouseUp(int theX, int theY)
{	
