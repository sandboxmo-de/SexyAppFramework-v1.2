...
std::string					SEHCatcher::mSubmitMessage = "Please help out by providing as much information as you can about this crash.";
std::string					SEHCatcher::mSubmitErrorMessage = "Failed to connect to server.";
...
{
	mImageHelpLib = LoadLibrary("IMAGEHLP.DLL");
    if (!mImageHelpLib)
        return false;
...
void SEHCatcher::GetSymbolsFromMapFile(std::string &theDebugDump)
{
	...
	HANDLE aFindHandle = FindFirstFile("*.map",&aFindData);
	...
	case WM_TIMER:
		{
			if (wParam == 0)
			{
					...
					(aResult != HTTPTransfer::RESULT_NOT_STARTED))
				{
						...
						(aResult != HTTPTransfer::RESULT_ABORTED))
					{
						...
						if (::MessageBox(hWnd, SEHCatcher::mSubmitErrorMessage.c_str(), 
							"Connection Failed", MB_RETRYCANCEL | MB_APPLMODAL) == IDRETRY)
						{
			...
			else if (wParam == 1)
			{
				...
				SetWindowText(gEditWindow, aNewString.c_str());
			}
		}
		break;
	}
...
static void CreateProgressWindow()
{
	...
	HWND aHWnd = CreateWindow("SEHProgressWindow", "Submitting Report",
		aWindowStyle,
		64, 64,
	...
	SEHCatcher::mNoButtonWindow = CreateWindow("BUTTON", "Abort", 
	...
	case WM_COMMAND:
		{
			...
			if (hwndCtl == mYesButtonWindow)
			{
				...
				char aStr[8192];
				GetWindowText(mEditWindow, aStr, 8192);
				mUserText = aStr;
...
void SEHCatcher::ShowSubmitInfoDialog()
{
	...
	HWND aHWnd = CreateWindow("SubmitInfoWindow", "Error Details",
		aWindowStyle,
		64+16, 64+16,
		aRect.right - aRect.left, 
		aRect.bottom - aRect.top,
	...
	HWND aLabelWindow = CreateWindow("EDIT", mSubmitMessage.c_str(),
	...
	HFONT aBoldArialFont = CreateFont(aFontHeight, 0, 0, 0, FW_BOLD, 0, 0,
	...
	HFONT aCourierNewFont = CreateFont(aFontHeight, 0, 0, 0, FW_NORMAL, 0, 0,
			false, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
	...
	mYesButtonWindow = CreateWindow("BUTTON", "Continue", 
		aWindowStyle, 
		aCurX, 300-24-8,
		aButtonWidth,
		24,
	...
	mNoButtonWindow = CreateWindow("BUTTON", "Abort", 
		WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON | BS_PUSHBUTTON, 
		aCurX, 300-24-8,
		aButtonWidth,
		24,
...
void SEHCatcher::ShowErrorDialog(const std::string& theErrorTitle, const std::string& theErrorText)
{
	...
	mDialogFont = ::CreateFont(aHeight, 0, 0, 0, FW_NORMAL, FALSE, FALSE,
			false, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
			DEFAULT_PITCH | FF_DONTCARE, "Tahoma");

	aHeight = -MulDiv(10, 96, 72);
	mBoldFont = ::CreateFont(aHeight, 0, 0, 0, FW_BOLD, FALSE, FALSE,
			false, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
			DEFAULT_PITCH | FF_DONTCARE, "Tahoma");	
	...
	HWND aHWnd = CreateWindow("SEHWindow", "Fatal Error!",
		aWindowStyle,
		64, 64,
		aRect.right - aRect.left, 
		aRect.bottom - aRect.top,
	...
	HWND aLabelWindow = CreateWindow("EDIT", 
		mCrashMessage.c_str(),
		WS_VISIBLE | WS_CHILD | ES_MULTILINE | ES_READONLY, 
		8, 8,
		400-8-8,
	...
	HWND anEditWindow = CreateWindow("EDIT", theErrorText.c_str(), 
	...
	if (canSubmit)
	{
		mYesButtonWindow = CreateWindow("BUTTON", "Send Report", 
			aWindowStyle, 
			aCurX, 300-24-8,
			aButtonWidth,
			24,
	...
	if (doDebugButton)
	{
		mDebugButtonWindow = CreateWindow("BUTTON", "Debug", 
			aWindowStyle, 
			aCurX, 300-24-8,
			aButtonWidth,
			24,
	...
	mNoButtonWindow = CreateWindow("BUTTON", "Close Now", 
		WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON | BS_PUSHBUTTON, 
		aCurX, 300-24-8,
		aButtonWidth,
		24,
...
std::string SEHCatcher::GetSysInfo()
{
	...
	if (mApp != NULL)
	{
		aMod = LoadLibrary("ddraw.dll");
		if (aMod != NULL)
		{
			GetModuleFileName(aMod, aPath, 256);			
			aDebugDump += "DDraw Ver: " + mApp->GetProductVersion(aPath) + "\r\n";
			FreeLibrary(aMod);
		}

		aMod = LoadLibrary("dsound.dll");
		if (aMod != NULL)
		{
			GetModuleFileName(aMod, aPath, 256);			
