...
int DDInterface::Init(HWND theWindow, bool IsWindowed)
{
	...
	else
	{
		...
		aResult = mDD->SetDisplayMode(mWidth, mHeight, mFullscreenBits);
...
bool DDInterface::Redraw(Rect* theClipRect)
{
	...
	Rect aScreenRect(0, 0, mWidth, mHeight);

	if (theClipRect == NULL)
		theClipRect = &aScreenRect;

	RECT aDestRect = theClipRect->ToRECT();
	RECT aSrcRect = theClipRect->ToRECT();
	POINT aPoint = {0, 0};		
...
void DDInterface::DrawCursorTo(LPDIRECTDRAWSURFACE theSurface, bool adjust)
{
	if ((mCursorImage != NULL) && (mPrimarySurface != NULL))
	{
		...
		Rect aSexyScreenRect(
			mCursorX - (mCursorWidth / 2),// + mCursorOffsetX,
			mCursorY - (mCursorHeight / 2),// + mCursorOffsetY,
			mCursorWidth,
			mCursorHeight);
		...
		if (theSurface == mDrawSurface && !mIs3D)
		{
			...
			g.DrawImage(mCursorImage, 
				mCursorX - (mCursorWidth / 2) + (mCursorWidth - mCursorImage->mWidth)/2, // + mCursorOffsetX, 
				mCursorY - (mCursorHeight / 2) + (mCursorHeight - mCursorImage->mHeight)/2); //+ mCursorOffsetY);				
		}
		else
		{
...
void DDInterface::MoveCursorTo(LPDIRECTDRAWSURFACE theSurface, bool adjust, int theNewCursorX, int theNewCursorY)
{
	...
	if ((mCursorImage != NULL) && (mPrimarySurface != NULL))
	{
		...
		Rect aNewClippedScreenRect = aNewSexyScreenRect.Intersection(Rect(0, 0, mWidth, mHeight));
