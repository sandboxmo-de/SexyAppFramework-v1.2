...
{	
	if (mFont == NULL)
		mFont = new SysFont(mWidgetManager->mApp, "SansSerif", 10, false);
...
void EditWidget::ProcessKey(KeyCode theKey, char theChar)
{
	...
	else
	{
		...
		if ((theChar >= 32) && (theChar < 127) && (mFont->StringWidth(aString) > 0) && mEditListener->AllowChar(mId, theChar))
		{				
			if ((mHilitePos != -1) && (mHilitePos != mCursorPos))
			{
				// Replace selection with new character
				mString = mString.substr(0, min(mCursorPos, mHilitePos)) + theChar + mString.substr(max(mCursorPos, mHilitePos));
				mCursorPos = min(mCursorPos, mHilitePos);
				mHilitePos = -1;
			...
			else
			{
				...
				mString = mString.substr(0, mCursorPos) + theChar + mString.substr(mCursorPos);
...
void EditWidget::MouseDown(int x, int y, int theClickCount)
{
	...
	std::string &aString = GetDisplayString();
	if ((theClickCount > 1) && (mCursorPos < (int) aString.length()))
	{
...
void EditWidget::MouseUp(int x, int y, int theClickCount)
{
