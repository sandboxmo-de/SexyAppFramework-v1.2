...
SexyAppBase::SexyAppBase()
{
	...
	gVersionDLL = LoadLibrary("version.dll");
	gDDrawDLL = LoadLibrary("ddraw.dll");
	gDSoundDLL = LoadLibrary("dsound.dll");
	gGetLastInputInfoFunc = (GetLastInputInfoFunc) GetProcAddress(GetModuleHandle("user32.dll"),"GetLastInputInfo");
	...
	char aFileName[256];
	GetModuleFileName(NULL, aFileName, 256);	
	mProductVersion = GetProductVersion(aFileName);	
			...
			if(!Is3DAccelerationRecommended()) // may need to prompt user if he wants to keep 3d acceleration on
			{
				if (Is3DAccelerated())
				{
					...
					int aResult = MessageBox(NULL,
						...
						(mCompanyName + " Hardware Acceleration Confirmation").c_str(), MB_YESNO | MB_ICONQUESTION);
...
bool SexyAppBase::ReadDemoBuffer(std::string &theError)
{
	...
	mHandleToIntMap.clear();
	
	mDemoPlayFrame = mUpdateCount;
	...
	fread(aStr, 1, aStrLen, aFP);
	aStr[aStrLen] = '\0';
	mDemoCommentStr = aStr;
	
	fread(&mDemoCommentDec, 4, 1, aFP);
...
void SexyAppBase::WriteDemoBuffer()
{
	if (mRecordingDemoBuffer)
	{
		...
		if (aFP != NULL)
		{
			...
			aStrLen = mDemoCommentStr.length();
			fwrite(&aStrLen, 2, 1, aFP);
			fwrite(mDemoCommentStr.c_str(), 1, mDemoCommentStr.length(), aFP);
			
			fwrite(&mDemoCommentDec, 4, 1, aFP);
			...
			ulong aDemoLength = mUpdateCount - mDemoPlayFrame;
...
void SexyAppBase::DemoAddMarker(const std::string& theString)
{
	...
	{
		mDemoMarkerList.push_back(DemoMarker(theString, mUpdateCount - mDemoPlayFrame));
	}
}
...
{	
	Dialog* aDialog = new Dialog(NULL, NULL, theDialogId, isModal, theDialogHeader,
...
bool SexyAppBase::OpenURL(const std::string& theURL, bool shutdownOnOpen)
{
	if ((!mIsOpeningURL) || (theURL != mOpeningURL))
	{
		...
		if ((int) ShellExecute(NULL, "open", theURL.c_str(), NULL, NULL, SW_SHOWNORMAL) > 32)
		{
			return true;
		}
		else
...
void SexyAppBase::TakeScreenshot()
{
	...
	// Get free image name
	std::string anImageDir = "_screenshots";
	mkdir("_screenshots");
	anImageDir += "/";
	...
	HANDLE aHandle = FindFirstFile((anImageDir + "*.png").c_str(), &aData);
...
void SexyAppBase::DumpProgramInfo()
{
	...
	while (aSortedItr != aSortedImageMap.end())
	{
		...
		aDumpStream << "<TD>" << ((aBitsMemory != 0) ? StrFormat("mBits<BR>%s", CommaSeperate(aBitsMemory).c_str()).c_str() : "&nbsp;") << "</TD>" << std::endl;	
		aDumpStream << "<TD>" << ((aPalletizedMemory != 0) ? StrFormat("Palletized<BR>%s", CommaSeperate(aPalletizedMemory).c_str()).c_str() : "&nbsp;") << "</TD>" << std::endl;				
		aDumpStream << "<TD>" << ((aSurfaceMemory != 0) ? StrFormat("DDSurface<BR>%s", CommaSeperate(aSurfaceMemory).c_str()).c_str() : "&nbsp;") << "</TD>" << std::endl;		
		aDumpStream << "<TD>" << ((aMemoryImage->mD3DData!=NULL) ? StrFormat("Texture<BR>%s<BR>%s", aTextureFormatName.c_str(), CommaSeperate(aTextureMemory).c_str()).c_str() : "&nbsp;") << "</TD>" << std::endl;

		aDumpStream << "<TD>" << ((aMemoryImage->mIsVolatile) ? "Volatile" : "&nbsp;") << "</TD>" << std::endl;
		aDumpStream << "<TD>" << ((aMemoryImage->mForcedMode) ? "Forced" : "&nbsp;") << "</TD>" << std::endl;		
		aDumpStream << "<TD>" << ((aMemoryImage->mHasAlpha) ? "HasAlpha" : "&nbsp;") << "</TD>" << std::endl;
		aDumpStream << "<TD>" << ((aMemoryImage->mHasTrans) ? "HasTrans" : "&nbsp;") << "</TD>" << std::endl;
		aDumpStream << "<TD>" << ((aNativeAlphaMemory != 0) ? StrFormat("NativeAlpha<BR>%s", CommaSeperate(aNativeAlphaMemory).c_str()).c_str() : "&nbsp;") << "</TD>" << std::endl;
		aDumpStream << "<TD>" << ((aRLAlphaMemory != 0) ? StrFormat("RLAlpha<BR>%s", CommaSeperate(aRLAlphaMemory).c_str()).c_str() : "&nbsp;") << "</TD>" << std::endl;
		aDumpStream << "<TD>" << ((aRLAdditiveMemory != 0) ? StrFormat("RLAdditive<BR>%s", CommaSeperate(aRLAdditiveMemory).c_str()).c_str() : "&nbsp;") << "</TD>" << std::endl;
		...
		ImageLib::WriteJPEGImage((std::string("_dump\\") + aThumbName).c_str(), &anImageLibImage);
		...
		ImageLib::WritePNGImage((std::string("_dump\\") + anImageName).c_str(), &anFullImage);
...
bool SexyAppBase::RegistryWrite(const std::string& theValueName, ulong theType, const uchar* theValue, ulong theLength)
{
	...
	int aResult = RegOpenKeyEx(HKEY_LOCAL_MACHINE, aKeyName.c_str(), 0, KEY_WRITE, &aGameKey);
	if (aResult != ERROR_SUCCESS)
	{
		ulong aDisp;
		aResult = RegCreateKeyEx(HKEY_LOCAL_MACHINE, aKeyName.c_str(), 0, "Key", REG_OPTION_NON_VOLATILE,
	...
	RegSetValueEx(aGameKey, aValueName.c_str(), 0, theType, theValue, theLength);
...
bool SexyAppBase::RegistryEraseKey(const std::string& theKeyName)
{
	...
	int aResult = RegDeleteKey(HKEY_LOCAL_MACHINE, aKeyName.c_str());
	if (aResult != ERROR_SUCCESS)
	{
		if (mRecordingDemoBuffer)
		{
...
void SexyAppBase::RegistryEraseValue(const std::string& theValueName)
{
	...
	if (aResult == ERROR_SUCCESS)
	{		
		RegDeleteValue(aGameKey, aValueName.c_str());
		RegCloseKey(aGameKey);
	}
}
...
bool SexyAppBase::RegistryGetSubKeys(const std::string& theKeyName, StringVector* theSubKeys)
{
	...
	else
	{
		...
		int aResult = RegOpenKeyEx(HKEY_LOCAL_MACHINE, aKeyName.c_str(), 0, KEY_READ, &aKey);
		...
		{		
			for (int anIdx = 0; ; anIdx++)
			{
				...
				aResult = RegEnumKey(aKey, anIdx, aStr, 1024);
				if (aResult != ERROR_SUCCESS)
					break;
...
bool SexyAppBase::RegistryRead(const std::string& theValueName, ulong* theType, uchar* theValue, ulong* theLength)
{
	...
	{		
		...
		if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, aKeyName.c_str(), 0, KEY_READ, &aGameKey) == ERROR_SUCCESS)
		{
			if (RegQueryValueEx(aGameKey, aValueName.c_str(), 0, theType, (uchar*) theValue, theLength) == ERROR_SUCCESS)
			{
				if (mRecordingDemoBuffer)
				{
					WriteDemoTimingBlock();
...
bool SexyAppBase::EraseFile(const std::string& theFileName)
{
	...
	return DeleteFile(theFileName.c_str()) != 0;
}
...
static void CalculateFPS()
{
	...
	if (gFPSTimer.GetDuration() >= 1000 || gForceDisplay)
	{
		...
		std::string aFPS = StrFormat("FPS: %d", gFPSDisplay);
...
static void FPSDrawCoords(int theX, int theY)
{
	...
	Graphics aDrawG(gFPSImage);
	aDrawG.SetFont(&aFont);
	std::string aFPS = StrFormat("%d,%d",theX,theY);
	aDrawG.SetColor(0x000000);
	aDrawG.FillRect(0,0,gFPSImage->GetWidth(),gFPSImage->GetHeight());
	aDrawG.SetColor(0xFFFFFF);
	aDrawG.DrawString(aFPS,2,aFont.GetAscent());	
...
static void CalculateDemoTimeLeft()
{
	...
	int a = gSexyAppBase->mUpdateCount;
	int b = gSexyAppBase->mDemoPlayFrame;
	int c = gSexyAppBase->mDemoLength;
	
	int aNumUpdatesLeft = gSexyAppBase->mDemoLength - (gSexyAppBase->mUpdateCount - gSexyAppBase->mDemoPlayFrame);
	...
	std::string aFPS = StrFormat("%02d:%02d:%02d", anHours,aMinutes,aSeconds);
	aDrawG.SetColor(0x000000);
...
void SexyAppBase::Popup(const std::string& theString)
{
	...
	BeginPopup();
	if (!mShutdown)
		::MessageBox(mHWnd, theString.c_str(), "FATAL ERROR", MB_APPLMODAL | MB_ICONSTOP);
	EndPopup();
}
...
BOOL CALLBACK EnumCloseThing2(HWND hwnd, LPARAM lParam)
{
	//CloseWindow(hwnd);
	char aClassName[256];
	if (GetClassName(hwnd, aClassName, 256) != 0)
	{
...
static INT_PTR CALLBACK MarkerListDialogProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
		case WM_INITDIALOG:
		{
			...
			{
				if (anItr->second <= gSexyAppBase->mUpdateCount - gSexyAppBase->mDemoPlayFrame)
					continue;
				...
				std::string aStr = StrFormat("%s (%02d:%02d:%02d)", anItr->first.c_str(),anHours,aMinutes,aSeconds);				
		...
		case WM_COMMAND:
			if (HIWORD(wParam)==LBN_DBLCLK)
			{
				...
				if (anIndex >= 0)
				{
					...
					if (anUpdateTime > gSexyAppBase->mUpdateCount - gSexyAppBase->mDemoPlayFrame)
					{
						gSexyAppBase->mFastForwardToUpdateNum = anUpdateTime + gSexyAppBase->mDemoPlayFrame;
						EndDialog(hwnd,0);
					}
				}
				return TRUE;
...
static INT_PTR CALLBACK JumpToTimeDialogProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
		case WM_INITDIALOG:
		{
			...
			if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, aKeyName.c_str(), 0, KEY_READ | KEY_WRITE, &aGameKey) == ERROR_SUCCESS)
			{
				char aBuf[1024];
				DWORD aLength = 1000;
				DWORD aType = REG_SZ;
				if (RegQueryValueEx(aGameKey, "DemoJumpTime", 0, &aType, (uchar*) aBuf, &aLength) == ERROR_SUCCESS)
				{
					aBuf[aLength] = 0;
					SetWindowText(anEdit,aBuf);
		...
		case WM_COMMAND:
			if (HIWORD(wParam)==BN_CLICKED)
			{
				if (LOWORD(wParam)==IDOK)
				{
					char aBuf[512];
					HWND anEdit = GetDlgItem(hwnd,100);
					GetWindowText(anEdit,aBuf,500);

					HKEY aGameKey;
					std::string aKeyName = RemoveTrailingSlash("SOFTWARE\\" + gSexyAppBase->mRegKey);
					if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, aKeyName.c_str(), 0, KEY_READ | KEY_WRITE, &aGameKey) == ERROR_SUCCESS)
					{
						RegSetValueEx(aGameKey, "DemoJumpTime", 0, REG_SZ, (const BYTE*)aBuf, strlen(aBuf)+1);
						RegCloseKey(aGameKey);
					}
...
//					gSexyAppBase->mFastForwardToUpdateNum = gSexyAppBase->mDemoLength - aNumFrames;					
					gSexyAppBase->mFastForwardToUpdateNum = gSexyAppBase->mDemoLength - aNumFrames + gSexyAppBase->mDemoPlayFrame;
...
/*			if (aSexyApp!=NULL && aSexyApp->mProcessInTimer && !aSexyApp->mShutdown && aSexyApp->mRunning)
			...
			if ((aSexyApp != NULL) && (!aSexyApp->mNoDefer))
			{
				...
				{
					if ((!aSexyApp->mForceFullscreen) && (!aSexyApp->mForceWindowed) && (wParam == VK_RETURN) /*&& (aSexyApp->mLoaded)*/)
					{
						if (pushMessage && !aSexyApp->mDeferredMessages.empty())
							aSexyApp->mDeferredMessages.pop_back(); // don't pass the enter key on to the game

						aSexyApp->SwitchScreenMode(!aSexyApp->mIsWindowed);
						aSexyApp->ClearKeysDown();
					}
...
/*	case WM_DISPLAYCHANGE:
	...
	if ((aSexyApp != NULL) && (uMsg == aSexyApp->mNotifyGameMessage))
	{
		...
		GetWindowPlacement(aSexyApp->mHWnd, &aWindowPlacement);		

		if (aWindowPlacement.showCmd == SW_SHOWMINIMIZED)
			ShowWindow(aSexyApp->mHWnd, SW_RESTORE);

		::SetForegroundWindow(aSexyApp->mHWnd);
		...
		while ((!mShutdown) && (mUpdateCount == mLastDemoUpdateCnt) && (!mDemoBuffer.AtEnd()))
		{
			if (PrepareDemoCommand(false))
			{
				...
				{				
					...
					case DEMO_KEY_CHAR:
						{
							char aChar = (char) mDemoBuffer.ReadNumBits(8, false);
...
#ifndef RELEASEFINAL
		if (ReparseModValues())
			PlaySound("c:\\windows\\media\\Windows XP Menu Command.wav", NULL, SND_ASYNC);				
		else
		{
			...
			case WM_CHAR:
				{
					...
					mDemoBuffer.WriteNumBits(aChar, 8);
				}				
				break;	
			case WM_CLOSE:
...
//					mActive = uMsg==WM_SETFOCUS;
			...
			case WM_MOUSEMOVE:		
				if ((!gInAssert) && (!mSEHOccured))
				{
					...
					mWidgetManager->MouseMove((short) LOWORD(lParam), (short) HIWORD(lParam));		
					...
					case WM_LBUTTONDOWN:		
						SetCapture(hWnd);
						mWidgetManager->MouseDown((short) LOWORD(lParam), (short) HIWORD(lParam), 1);						
						break;
					case WM_RBUTTONDOWN:
						SetCapture(hWnd);
						mWidgetManager->MouseDown((short) LOWORD(lParam), (short) HIWORD(lParam), -1);						
					...
					case WM_MBUTTONDOWN:
						SetCapture(hWnd);
						mWidgetManager->MouseDown((short) LOWORD(lParam), (short) HIWORD(lParam), 3);						
						break;
					case WM_LBUTTONDBLCLK:
						SetCapture(hWnd);
						mWidgetManager->MouseDown((short) LOWORD(lParam), (short) HIWORD(lParam), 2);						
						break;
					case WM_RBUTTONDBLCLK:
						SetCapture(hWnd);
						mWidgetManager->MouseDown((short) LOWORD(lParam), (short) HIWORD(lParam), -2);						
					...
					case WM_LBUTTONUP:		
						if ((mWidgetManager->mDownButtons & ~1) == 0)
							ReleaseCapture();
						mWidgetManager->MouseUp((short) LOWORD(lParam), (short) HIWORD(lParam), 1);						
						break;
					case WM_RBUTTONUP:		
					...
					case WM_MBUTTONUP:		
						if ((mWidgetManager->mDownButtons & ~4) == 0)
							ReleaseCapture();
						mWidgetManager->MouseUp((short) LOWORD(lParam), (short) HIWORD(lParam), 3);								
						break;
					}
				}
				break;		
			...
			case WM_TIMER:
				...
				{	
					...
					static POINT prevCursor = {-1, -1}; 
					if(mHasFocus && prevCursor.x != -1) 
					{ 
						int xDiff = aPoint.x - prevCursor.x; 
						int yDiff = aPoint.y - prevCursor.y; 

						aPoint.x = prevCursor.x + (int)(xDiff*mMouseSensitivity); 
						aPoint.y = prevCursor.y + (int)(yDiff*mMouseSensitivity); 
					...
					if (mMouseIn != isMouseIn)
					{
						...
						if (!isMouseIn)
							mWidgetManager->MouseExit(aPoint.x - aULCorner.x, aPoint.y - aULCorner.y);
...
void SexyAppBase::MakeWindow()
{
	...
	if ((mIsWindowed) && (aResult == DDInterface::RESULT_INVALID_COLORDEPTH))
	{
		if (mForceWindowed)
		{
			Popup("Please set your desktop color depth to 16 bit.");
			DoExit(1);
		}
		else
		{
	...
	else if (aResult != DDInterface::RESULT_OK)
	{
		...
		{
			Popup("Failed to initialize DirectDraw: " + DDInterface::ResultToString(aResult) + " " + mDDInterface->mErrorString);
			DoExit(1);
		}
	}
...
bool SexyAppBase::UpdateAppStep(bool* updated)
{
	...
	// when paused, process only windows messages
	if (mPaused)
	{
		mUpdateAppState = UPDATESTATE_PROCESS_DONE;
		Sleep(100);
...
bool SexyAppBase::LoadProperties(const std::string& theFileName, bool required, bool checkSig)
{
	...
	if (!ReadBufferFromFile(theFileName, &aBuffer))
	{
		...
		{
			Popup("Unable to open properties file: " + theFileName);
			return false;
		}
	}
	...
	if (checkSig)
	{
		...
		{
			Popup("Signature check failed on '" + theFileName + "'");
			return false;
		}
	}
...
{
	WIN32_FIND_DATA aData;
	HANDLE aHandle = FindFirstFile((theDemoPrefix + "*.dmo").c_str(), &aData);
	if (aHandle==INVALID_HANDLE_VALUE)
		return 0;
...
void SexyAppBase::HandleCmdLineParam(const std::string& theParamName, const std::string& theParamValue)
{
	...
	{
		Popup("Invalid Command Line Param: " + theParamName);
		DoExit(0);
	}
}
...
void SexyAppBase::Init()
{
	...
	{
		MessageBox(NULL,"This application requires DirectX to run.  You can get DirectX at http://www.microsoft.com/directx","You need DirectX",MB_OK | MB_ICONERROR);
		DoExit(0);
	}	
	...
	if(mOnlyAllowOneCopyToRun)
	{
		...
		mNotifyGameMessage = RegisterWindowMessage(("Notify" + mProdName).c_str());

		// Create a globally unique mutex to make sure we are the only burrito
		HANDLE aMutex = CreateMutex(NULL, TRUE, (mProdName + "Mutex").c_str());
	...
	char* aSlashPos = max(strrchr(aPath, '\\'), strrchr(aPath, '/'));
	if (aSlashPos != NULL)
		*aSlashPos = '\0';

	if (!ChangeDirHook(aPath))
		chdir(aPath);
	...
	wc.hCursor = NULL;
	wc.hIcon = ::LoadIcon(gHInstance, "IDI_MAIN_ICON");
