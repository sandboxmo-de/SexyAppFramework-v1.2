...
ResourceManager::ResourceManager(SexyAppBase *theApp) 
{
	...
	mImageItr = mImageMap.end();
	mSoundItr = mSoundMap.end();
	mFontItr = mFontMap.end();
...
bool ResourceManager::ParseImageResource(XMLElement &theElement)
{
	...
	if (anItr != theElement.mAttributes.end())
	{
		...
		if (stricmp(aType,"none")==0) anAnimType = AnimType_None;
		else if (stricmp(aType,"once")==0) anAnimType = AnimType_Once;
		else if (stricmp(aType,"loop")==0) anAnimType = AnimType_Loop;
		else if (stricmp(aType,"pingpong")==0) anAnimType = AnimType_PingPong;
		else 
		{
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
bool ResourceManager::LoadNextImage()
{
	if (HadError())
		return false;

	for (; mImageItr != mImageMap.end(); ++mImageItr)
	{
		ImageRes *aRes = (ImageRes*)mImageItr->second;
		if (!mCurResGroup.empty() && aRes->mResGroup!=mCurResGroup)
			continue;

		if ((DDImage*) aRes->mImage != NULL || aRes->mFromProgram)
			continue;
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
bool ResourceManager::LoadNextSound()
{
	if (HadError())
		return false;

	for (; mSoundItr != mSoundMap.end(); ++mSoundItr)
	{
		SoundRes *aRes = (SoundRes*)mSoundItr->second;
		if (!mCurResGroup.empty() && aRes->mResGroup!=mCurResGroup)
			continue;

		if (aRes->mSoundId!=-1 || aRes->mFromProgram)
			continue;

		SEXY_PERF_BEGIN("ResourceManager:LoadSound");
		if(!mApp->mSoundManager->LoadSound(mCurSoundId, aRes->mPath))
			return Fail(StrFormat("Failed to load sound: %s",aRes->mPath.c_str()));
		SEXY_PERF_END("ResourceManager:LoadSound");

		if (aRes->mVolume >= 0)
			mApp->mSoundManager->SetBaseVolume(mCurSoundId, aRes->mVolume);

		if (aRes->mPanning != 0)
			mApp->mSoundManager->SetBasePan(mCurSoundId, aRes->mPanning);

		aRes->mSoundId = mCurSoundId++;
		return true;
	}
...
{
	ImageFont *aFont = NULL;

	SEXY_PERF_BEGIN("ResourceManager:DoLoadFont");

	if (theRes->mImagePath.empty())	
	{
		if (strncmp(theRes->mPath.c_str(),"!ref:",5)==0)
		{
			...
			aFont = (ImageFont*)aRefFont->Duplicate();
		}
		else
	...
	if (aFont->mFontData==NULL || !aFont->mFontData->mInitialized)
	{
		delete aFont;
...
bool ResourceManager::LoadNextFont()
{
	if (HadError())
		return false;

	for (; mFontItr != mFontMap.end(); ++mFontItr)
	{
		FontRes *aRes = (FontRes*)mFontItr->second;
		if (!mCurResGroup.empty() && aRes->mResGroup!=mCurResGroup)
			continue;

		if (aRes->mFont != NULL || aRes->mFromProgram)
			continue;
...
void ResourceManager::StartLoadResources(const std::string &theGroup)
{
	...
	mImageItr = mImageMap.begin();
	mSoundItr = mSoundMap.begin();
	mFontItr = mFontMap.begin();
}
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
const StringToStringMap& ResourceManager::GetImageAttributes(const std::string &theId)
{
	static StringToStringMap aStrMap;
