...
bool FontData::HandleCommand(const ListDataElement& theParams)	
{
	...
	else if (stricmp(aCmd.c_str(), "LayerSetColorMult") == 0)
	{
		if (theParams.mElementVector.size() == 3)
		{
			...
			if ((DataToLayer(theParams.mElementVector[1], &aLayer)) && 
				(DataToDoubleVector(theParams.mElementVector[2], &aFactorVector)) &&
				(aFactorVector.size() == 4))
			{
				aLayer->mColorMult = Color(
...
bool FontData::LoadLegacy(Image* theFontImage, const std::string& theFontDescFileName)
{
	...
	int aCharPos = 0;
	std::ifstream aStream(theFontDescFileName.c_str(), std::ios::in);
	...
	int aSpaceWidth = 0;
	aStream >> aFontLayer->mCharData[' '].mWidth >> aFontLayer->mAscent;
...
void ImageFont::DrawStringEx(Graphics* g, int theX, int theY, const std::string& theString, const Color& theColor, const Rect* theClipRect, RectList* theDrawnAreas, int* theWidth)
{
