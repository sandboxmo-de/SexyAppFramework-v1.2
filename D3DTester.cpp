...
bool D3DTester::CheckRegistry()
{
	...
	aType = REG_DWORD;
	if (RegQueryValueEx(mRegKey, "Version", 0, &aType, (uchar*) &aVersion, &aSize) != ERROR_SUCCESS)
		return false;
	...
	aType = REG_DWORD;
	if (RegQueryValueEx(mRegKey, "MinVidMemory", 0, &aType, (uchar*) &aMinVidMemory, &aSize) != ERROR_SUCCESS)
		return false;
	...
	aType = REG_DWORD;
	if (RegQueryValueEx(mRegKey, "RecVidMemory", 0, &aType, (uchar*) &aRecVidMemory, &aSize) != ERROR_SUCCESS)
		return false;
	...
	aSize = sizeof(aGUID);
	aType = REG_BINARY;
	if (RegQueryValueEx(mRegKey, "DisplayGUID", 0, &aType, (uchar*) &aGUID, &aSize) != ERROR_SUCCESS)
		return false;
	...
	aType = REG_SZ;
	aSize = 4096;
	if (RegQueryValueEx(mRegKey, "FailureReason", 0, &aType, (uchar*) aBuf, &aSize) != ERROR_SUCCESS)
		return false;
	...
	aType = REG_SZ;
	aSize = 4096;
	if (RegQueryValueEx(mRegKey, "Warning", 0, &aType, (uchar*) aBuf, &aSize) != ERROR_SUCCESS)
		return false;
...
void D3DTester::WriteToRegistry()
{
	...
	aSize = sizeof(aVersion);
	aType = REG_DWORD;
	RegSetValueEx(mRegKey, "Version", 0, aType, (uchar*) &aVersion, aSize);

	// Write Min Vid Memory
	DWORD aMinVidMemory = mMinVidMemory;
	aSize = sizeof(aMinVidMemory);
	aType = REG_DWORD;
	RegSetValueEx(mRegKey, "MinVidMemory", 0, aType, (uchar*) &aMinVidMemory, aSize);

	// Write Recommended Vid Memory
	DWORD aRecVidMemory = mRecommendedVidMemory;
	aSize = sizeof(aRecVidMemory);
	aType = REG_DWORD;
	RegSetValueEx(mRegKey, "RecVidMemory", 0, aType, (uchar*) &aRecVidMemory, aSize);

	// Write GUID
	aSize = sizeof(mDisplayGUID);
	aType = REG_BINARY;
	RegSetValueEx(mRegKey, "DisplayGUID", 0, aType, (uchar*) &mDisplayGUID, aSize);

	// Write failure reason
	aType = REG_SZ;
	aSize = mFailureReason.length()+1;
	RegSetValueEx(mRegKey, "FailureReason", 0, aType, (uchar*) mFailureReason.c_str(), aSize);

	// Write warining
	aType = REG_SZ;
	aSize = mWarning.length()+1;
	RegSetValueEx(mRegKey, "Warning", 0, aType, (uchar*) mWarning.c_str(), aSize);
}
...
bool D3DTester::Init(HWND theHWND, LPDIRECTDRAW7 theDDraw)
{
	...
	if (mCheckRegistry)
	{
		...
		RegCreateKeyEx(HKEY_LOCAL_MACHINE, aKey.c_str(),0,"",REG_OPTION_NON_VOLATILE,KEY_ALL_ACCESS,NULL,&mRegKey,NULL);
	}
	...
	try
	{
		...
		if (aPath.find_first_of("/\\")==std::string::npos)
		{
			...
			if (GetSystemDirectory(aBuf,sizeof(aBuf)-1))
				aPath = AddTrailingSlash(aBuf,true) + aPath;
		}

		FILETIME aFileTime;
		memset(&aFileTime, 0, sizeof(aFileTime));
		HANDLE aFileHandle = CreateFile(aPath.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
...
void D3DTester::TestD3D(HWND theHWND, LPDIRECTDRAW7 theDDraw)
{
	if (!gSexyAppBase->mPlayingDemoBuffer)
	{
		...
		else
		{
			...
			{
				mResultsChanged = RegQueryValueEx(mRegKey, "FailureReason", 0, NULL, NULL, NULL) != ERROR_SUCCESS;
				mShouldWriteToRegistry = true;
			}
		}
