...
void WidgetContainer::RemoveWidget(Widget* theWidget)
{
	...
	{								
		if (mWidgetManager != NULL)
		{
			mWidgetManager->DisableWidget(theWidget);
			...
			while (anItr != mWidgetManager->mPreModalInfoList.end())
			{
				...
				if (aPreModalInfo->mPrevBaseModalWidget == theWidget)
					aPreModalInfo->mPrevBaseModalWidget = NULL;
				if (aPreModalInfo->mPrevFocusWidget == theWidget)
		...
		if (mWidgetManager != NULL)
		{
			theWidget->RemovedFromManager(mWidgetManager);
...
{	
	...
	ModFlags(aCurFlags, mWidgetFlagsMod);
...
void WidgetContainer::BringToBack(Widget* theWidget)
{
	...
	if (anItr != mWidgets.end())
	{
		...
		mWidgets.erase(anItr);
		mWidgets.insert(mWidgets.begin(), theWidget);
