...
void Graphics::DrawImageTransformHelper(Image* theImage, const Transform &theTransform, const Rect &theSrcRect, float x, float y, bool useFloat)
{
	...
	else if (theTransform.mHaveScale)
	{
		...
		DrawImage(theImage,aDestRect,theSrcRect);
	}
	else
	{
...
int Graphics::WriteString(const std::string& theString, int theX, int theY, int theWidth, int theJustification, bool drawString, int theOffset, int theLength)
{
	Font* aFont = GetFont();
	...
	if (drawString)
	{
		...
		{	
		case 0:
			theX += (theWidth - WriteString(theString, theX, theY, theWidth, -1, false, theOffset, theLength))/2;
			break;
		case 1:
			theX += theWidth - WriteString(theString, theX, theY, theWidth, -1, false, theOffset, theLength);
			break;
		}
	}
	...
	for (int i = theOffset; i < theLength; i++)
	{
		...
		{
			if (i > theLength-8) // badly formatted color specification
				break;
			else if (theString[i+1] == '^') // literal '^'
			{
				aString += '^';
				i++;
			}
...
int	Graphics::WriteWordWrapped(const Rect& theRect, const std::string& theLine, int theLineSpacing, int theJustification, int *theMaxWidth)
{
	...
	{	
		...
		if(aCurWidth > theRect.mWidth) // need to wrap
		{
			...
			if(aSpacePos!=-1)
			{
				aWrittenWidth = WriteString(theLine, theRect.mX, theRect.mY + aYOffset, theRect.mWidth, 
			...
			else
			{
				...
				aWrittenWidth = WriteString(theLine, theRect.mX, theRect.mY + aYOffset, theRect.mWidth, 
					theJustification, true, aLineStartPos, aCurPos-aLineStartPos);
	...
	{
		int aWrittenWidth = WriteString(theLine, theRect.mX, theRect.mY + aYOffset, theRect.mWidth, 
			theJustification, true, aLineStartPos, theLine.length()-aLineStartPos);
